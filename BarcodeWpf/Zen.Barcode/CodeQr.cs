...
namespace Zen.Barcode
{
    ...
    public class CodeQrBarcodeDraw : BarcodeDraw
	{
        ...
        private class QRCodeEncoder
		{
			...
			QrErrorCorrection _errorCorrect;
			QrEncodeMode _encodeMode;
			int _version;
            CodeQrBarcodeDraw _owner;

            int _structureAppendN;
			int _structureAppendM;
			int _structureAppendParity;
			//string _structureAppendOriginaldata;
			...
			/// Constructor
			/// </summary>
			public QRCodeEncoder(CodeQrBarcodeDraw owner)
			{
                _owner = owner;
            ...
            /// </returns>
            public virtual DrawingGroup Encode(string content, Encoding encoding)
            {
                ...
                var width = (matrix.Length * _scale) + 1;
                var height = (matrix.Length * _scale) + 1;
                var drawingGroup = new DrawingGroup();
                using (var dc = drawingGroup.Open())
                {
                    dc.DrawRectangle(_owner.Background, null, new Rect(0, 0, width - 1, height - 1));
                    for (int i = 0; i < matrix.Length; i++)
                    {
                        for (int j = 0; j < matrix.Length; j++)
                        {
                            if (matrix[j][i])
                            {
                                dc.DrawRectangle(_owner.Foreground, null, new Rect(j * _scale, i * _scale, _scale, _scale));
                            }
                        }
                    }
                }
			...
			/// A <see cref="Bitmap"/> object representing the QR barcode.
			/// </returns>
			public virtual DrawingGroup Encode(string content)
			{
		...
		{
			QRCodeEncoder temp = new QRCodeEncoder(this);
		...
		/// <returns></returns>
		protected virtual DrawingGroup DrawQr(string text, BarcodeMetricsQr metrics)
		{
			QRCodeEncoder encoder = new QRCodeEncoder(this)
			{
				Scale = metrics.Scale,
