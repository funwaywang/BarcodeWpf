...
namespace Zen.Barcode
{
    ...
    public class CodePdf417BarcodeDraw : BarcodeDraw
	{
		...
		private DrawingGroup _outImage;
		...
		/// <returns></returns>
		public override DrawingGroup Draw(string text, BarcodeMetrics metrics)
		{
            Metrics = metrics;
			_text = Filter(text);
			return PaintCode();
		...
		public virtual DrawingGroup PaintCode()
		{
        ...
        private void RenderCodeword(int codeword, int bits, RenderArgs args)
        {
            // defaultMetrics
            using (var dc = _outImage.Append())
            {
                var scale = Math.Max(1, Metrics.Scale);
                for (int bit = (bits - 1); bit >= 0; --bit)
                {
                    ...
                    //    pixel = Colors.Black;
                    //}
                    //_outImage.WritePixels(.SetPixel(_outPosition.X++, _outPosition.Y, pixel);
                    var flag = (codeword & (2 ^ bit)) != 0;
                    if (args.IsInverted)
                    {
                        flag = !flag;
                    }
                    ...
                    if (flag)
                    {
                        var rect = new Rect(args.X * scale, args.Y * scale, scale, scale);
		...
		private void RenderCodeword17(int codeword, RenderArgs args)
		{
			RenderCodeword(codeword, 17, args);
		}

		private void RenderCodeword18(int codeword, RenderArgs args)
		{
			RenderCodeword(codeword, 18, args);
		}

		private void RenderCodeword(int codeword, RenderArgs args)
		{
			RenderCodeword17(codeword, args);
		}

		private void RenderStopPattern(RenderArgs args)
		{
			RenderCodeword18(STOP_PATTERN, args);
		}

		private void RenderStartPattern(RenderArgs args)
		{
			RenderCodeword17(START_PATTERN, args);
		}

		private void RenderEdgePattern(RenderArgs args, bool start)
		{
            var row = args.Row;
			int rowMod = row % 3;
			int edge = 0;
			...
			RenderCodeword(cluster[edge], args);
		}

		private void RenderRowPattern(RenderArgs args)
		{
			int rowMod = args.Row % 3;
			int[] cluster = _clusters[rowMod];
			for (int column = 0; column < _codeColumns; ++column)
			{
				RenderCodeword(cluster[_codewords[args.CodePoint++]], args);
			}
		}

		private DrawingGroup PaintCodeInternal()
		{
			_bitColumns = START_CODE_SIZE * (_codeColumns + 3) + STOP_SIZE;
            //_outImage = new WriteableBitmap(_bitColumns, _codeRows, 96, 96, PixelFormats.Rgb24, BitmapPalettes.Gray16);
            _outImage = new DrawingGroup();
			//int codePtr = 0;

            var args = new RenderArgs();
            args.IsInverted = (_options & PdfRenderFlags.InvertBitmap) != 0;
            ...
            for (int row = 0; row < _codeRows; ++row)
			{
                ...
                //RenderRowPattern(row, ref codePtr);
                ...
                RenderStartPattern(args);
				RenderEdgePattern(args, true);
				RenderRowPattern(args);
				RenderEdgePattern(args, false);
				RenderStopPattern(args);
			}

            //if ((_options & PdfRenderFlags.InvertBitmap) != 0)
            //{
            //    // Render new image which is an XOR of the original
            //    var inverted = _outImage.Clone();
