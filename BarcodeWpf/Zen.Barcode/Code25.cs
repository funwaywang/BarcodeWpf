...
namespace Zen.Barcode
{
		...
		: BarcodeDrawBase<Code25GlyphFactory, Code25Checksum>
	{
		...
		/// </remarks>
		protected override void RenderBars(Glyph[] barcode, DrawingContext dc, Rect bounds, int interGlyphSpace, int barMinHeight, int barMinWidth, int barMaxWidth)
		{
			...
			for (int index = 0; index < barcode.Length; )
			{
				...
				else
				{
					...
					for (int bitIndex = encodingBitCount - 1; bitIndex >= 0; --bitIndex)
					{
						...
						{
                            dc.DrawRectangle(Foreground, null, new Rect(barOffset, bounds.Top, barMinWidth * 2, height));
