...
namespace Zen.Barcode
{
	...
	public class BarcodeMetrics1d : BarcodeMetrics
	{
		...
		int _minHeight;
		int _maxHeight;
		int? _interGlyphSpacing;
		bool _renderVertically;
		#endregion
		...
		public BarcodeMetrics1d(double width, double height)
		{
			_minWidth = _maxWidth = (int)Math.Floor(width);
            _minHeight = _maxHeight = (int)Math.Floor(height);
        }
        ...
        public BarcodeMetrics1d(double minWidth, double maxWidth, double height)
		{
            _minWidth = (int)Math.Floor(minWidth);
            _maxWidth = (int)Math.Floor(maxWidth);
            _minHeight = _maxHeight = (int)Math.Floor(height);
        }
        ...
        public BarcodeMetrics1d(double minWidth, double maxWidth, double minHeight, double maxHeight)
        {
            _minWidth = (int)Math.Floor(minWidth);
            _maxWidth = (int)Math.Floor(maxWidth);
            _minHeight = (int)Math.Floor(minHeight);
            _maxHeight = (int)Math.Floor(maxHeight);
        }
	...
	public abstract class BarcodeDraw
	{
		...
		/// An <see cref="Image"/> object containing the rendered barcode.
		/// </returns>
		public DrawingGroup Draw(string text, int maxBarHeight, int scale)
		{
			BarcodeMetrics defaultMetrics = GetDefaultMetrics(maxBarHeight);
        ...
        public DrawingImage DrawImage(string text, int maxBarHeight)
        {
            var drawing = Draw(text, maxBarHeight);
            RenderOptions.SetEdgeMode(drawing, EdgeMode.Aliased);
            var image = new DrawingImage(drawing);
            if (image.CanFreeze)
                image.Freeze();
        ...
        public DrawingImage DrawImage(string text, int maxBarHeight, int scale)
        {
            var drawing = Draw(text, maxBarHeight, scale);
            RenderOptions.SetEdgeMode(drawing, EdgeMode.Aliased);
            var image = new DrawingImage(drawing);
            if (image.CanFreeze)
                image.Freeze();
	...
	{
		#region Private Fields
		TGlyphFactory _factory;
		TChecksum _checksum;
		int _encodingBitCount;
		int _widthBitCount;
        #endregion
		...
		/// <param name="metrics">A <see cref="T:Zen.Barcode.BarcodeMetrics"/> object.</param>
		/// <returns></returns>
		public override sealed DrawingGroup Draw(string text, BarcodeMetrics metrics)
		{
			...
			int barMaxWidth)
		{
			...
			// Get glyph height
			int height = GetGlyphHeight(glyph, barMinHeight, (int)bounds.Height);
			if (glyph is IBinaryPitchGlyph)
			{
				...
				for (int bitIndex = encodingBitCount - 1; bitIndex >= 0; --bitIndex)
				{
                    ...
                    {
                        dc.DrawRectangle(Foreground, null, new Rect(barOffset, bounds.Top, barWidth, height));
                    }
			...
			else
			{
				for (int bitIndex = encodingBitCount - 1; bitIndex >= 0; --bitIndex)
				{
                    ...
                    {
                        dc.DrawRectangle(Foreground, null, new Rect(barOffset, bounds.Top, barMinWidth, height));
                    }
